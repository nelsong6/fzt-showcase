...
- Initial scaffold: WASM bridge in fzt, vanilla frontend with Tokyo Night theme, sample YAML tree, drill-down navigation
...
- **GitHub links in hero**: Added two links below the tagline — "GitHub" pointing to the main fzt tool repo (`nelsong6/fzt`) and "Source" pointing to the showcase repo (`nelsong6/fzt-showcase`). Styled with retro green color and phosphor glow on hover.
...
- **"fuzzy finder" links to fzf** — the intro text "A fuzzy finder with hierarchical navigation" now links "fuzzy finder" to `https://github.com/junegunn/fzf`. Added `{text}(url)` inline link syntax parser to the command history output renderer so links can be embedded in plain text output lines.

### 2026-04-04

- **WASM downloaded from release instead of built from source** — replaced the 3-step checkout+Go+build with `gh release download` from the latest fzt release. Eliminates the Go toolchain from this repo's CI entirely. Added `repository_dispatch` trigger (`fzt-updated`) so new fzt releases automatically redeploy the showcase.
